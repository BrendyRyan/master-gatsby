...
async function turnPizzasIntoPages({ graphql, actions }) {
  ...
  `);
  // 3. Loop over pizzas and create a page for that pizza.
...
async function turnToppingsIntoPages({ graphql, actions }) {
  ...
  `);
  // 3. Create pages for those toppings
...
async function fetchBeersAndTurnIntoNodes({
  ...
  for (const beer of beers) {
    // if no beer name (api error) than skip
    if (!beer.name) return;
    // create a node of each beer
    const nodeMeta = {
...
async function turnSlicemastersIntoPages({ graphql, actions }) {
  ...
  // 3. Figure out how many pages there are based on # of slicemasters there are and how many per page
